...
1. Split the data 80-20 and set aside a 20% {term}`test set <test data>`. THis can be uploaded to the course [huggingface org](https://huggingface.co/CSC310-fall25) as a dataset or uploaded with your model. 
...
3. Fit a linear model on all numerical features with 75% of your training data (60% of the whole dataset).
4. Test it on 25% held out {term}`validation data` and measure the fit with two metrics and one plot
...
:::::{tip}
See the [tutorial](../resources/sharing.md)
:::::::
...
::::{tip}
A thorough model card is a way to earn `innovative` for communicate.  
